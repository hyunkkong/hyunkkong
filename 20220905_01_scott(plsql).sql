...
-- 자동으로 파생된 열 값 생성 → 자동으로 열을 만들게 한다.
-- 잘못된 트랙잭션 방지
-- 복잡한 보안 권한 강제 수행
-- 분산 데이터베이스 노드 상에서 참조 무결성 강제 수행
-- 복잡한 업무 규칙 강제 적용 → 업무 시간 도중 쇼핑,증권 사이트 접근 X 규칙 강제 적용
-- 복잡한 감사 제공
-- 동기 테이블 복제 유지관리
-- 테이블 액세스 통계 수집
...
CREATE [OR REPLACE] TRIGGER 트리거명
    ...
    [FOR EACH ROW [WHEN TRIGGER 조건]] -- STATEMENT 트리거에는 사용 X
[DECLARE]
    -- 선언 구문;
BEGIN
    -- 실행 구문;
...
BEGIN
    ...
    */
    IF ( TO_NUMBER(TO_CHAR(SYSDATE,'HH24')) < 8
         OR TO_NUMBER(TO_CHAR(SYSDATE,'HH24')) > 17 )   -- 17:59분
...
BEGIN
    ...
    IF (INSERTING)
        THEN UPDATE TBL_상품
             SET 재고수량 = 기존 재고수량 + 새로 입고되는 상품의 재고수량
